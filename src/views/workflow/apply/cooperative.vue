<template>
     <div class="app-container">
        ...
        <el-dialog v-dialogDrag :title="operate" :visible.sync="formVisible" @close="closeForm(false)" width="1000px" destroy-on-close>
            <cooperative-form :operate="operate" :businessKey="row.id" @close="closeForm"></cooperative-form>
        </el-dialog>
        <!-- 撤回申请 -->
        ...
        <el-dialog v-dialogDrag :title="operate" :visible.sync="detailFormVisible" @close="closeForm(false)" width="1000px" destroy-on-close>
            <cooperative-detail :operate="operate" :businessKey="row.id" @close="closeForm"></cooperative-detail>
        </el-dialog>
    </div>
</template>

<script>
import api from '@/api/cooperative'
import apiIns from '@/api/instance'
import CooperativeDetail from './components/CooperativeDetail'
import CancelApply from './components/CancelApply'
import CooperativeForm from '@/components/Process/Form/CooperativeForm'
...
export default {
    ...
    methods: {
        ...
        window.open('http://localhost:9528/cbsp/download/'+row.id)
        },
