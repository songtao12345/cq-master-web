<template>
     <div class="app-container">
        ...
        <el-dialog v-dialogDrag :title="operate" :visible.sync="formVisible" @close="closeForm(false)" width="1000px" destroy-on-close>
            <crude-form :operate="operate" :businessKey="row.id" @close="closeForm"></crude-form>
        </el-dialog>
        <!-- 撤回申请 -->
        ...
        <el-dialog v-dialogDrag :title="operate" :visible.sync="detailFormVisible" @close="closeForm(false)" width="1000px" destroy-on-close>
            <crude-detail :operate="operate" :businessKey="row.id" @close="closeForm"></crude-detail>
        </el-dialog>
    </div>
</template>

<script>
import api from '@/api/crude'
import apiIns from '@/api/instance'
import CrudeDetail from './components/CrudeDetail'
import CancelApply from './components/CancelApply'
import CrudeForm from '@/components/Process/Form/CrudeForm'
...
export default {
    ...
    methods: {
        ...
        window.open('http://localhost:9528/cos/download/'+row.id)
        },
